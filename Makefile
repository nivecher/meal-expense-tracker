...
lint-yaml: check-env
	...
	@if command -v yamllint >/dev/null 2>&1; then \
		yamllint -c .yamllint **/*.yaml **/*.yml || (echo "\033[1;31m❌ YAML linting failed\033[0m"; exit 1); \
	elif [ -d node_modules ] && command -v npm >/dev/null 2>&1; then \
		npx --yes prettier --check "**/*.{yaml,yml}" || (echo "\033[1;31m❌ YAML validation failed\033[0m"; exit 1); \
	else \
